...
(function ($) {
	...
	$.ajaxJSONP = function (options, deferred) {
		...
		var _callbackName = options.jsonpCallback,
		    callbackName = ($.isFunction(_callbackName) ? _callbackName() : _callbackName) || 'Mobile' + jsonpID++,
				...
				if (xhr.status >= 200 && xhr.status < 300 || xhr.status == 304 || xhr.status == 0 && protocol == 'file:') {
					...
					if (xhr.responseType == 'arraybuffer' || xhr.responseType == 'blob') result = xhr.response;else {
						...
						try {
							...
							if (dataType == 'script') (eval)(result);else if (dataType == 'xml') result = xhr.responseXML;else if (dataType == 'json') result = blankRE.test(result) ? null : JSON.parse(result);
						} catch (e) {
							error = e;
						}
